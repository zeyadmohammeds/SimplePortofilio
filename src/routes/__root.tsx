...
function NotFoundComponent() {
  ...
  // Redirect paths that are missing their trailing slash
  ...
  React.useEffect(() => {
    const target = slashRedirects[location.pathname];
    if (target) navigate({ to: target as "/" });
  }, [location.pathname]);

  if (slashRedirects[location.pathname]) {
    return (
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="rounded-[2.5rem] bg-surface p-8 md:p-12 max-w-2xl text-center flex flex-col items-center">
        <div className="pill-label mb-8">
          <span className="h-2 w-2 rounded-full bg-primary" />
          404 / Route Not Found
        </div>
        <h1 className="font-display text-6xl font-black tracking-tight text-foreground sm:text-8xl mb-6">
          Lost Route
        </h1>
...
export const Route = createRootRoute({
  head: () => ({
    ...
    links: [
      { rel: "icon", href: "/public/me copy.png" },
